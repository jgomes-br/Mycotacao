from .models import Cotacao, Lance
...
class RespostaInterface:
    ...
    def __init__(self, dono, id_estrutura: int, valor: str) :
        self.dono = dono
        self.estrutura= Cotacao.objects.get(pk=id_estrutura)
        ...
        for produto in projeto.produtos.all():
            for fornecedor in projeto.fornecedores.all():
                if not Cotacao.objects.filter(projeto=projeto, produto=produto, fornecedor=fornecedor).exists():
                    Cotacao.objects.create(projeto=projeto, produto=produto, fornecedor=fornecedor, status='1').save()
